...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (last > left) {
            ...
            int leftRight = order.compare(array.entry(left),
                    array.entry(right));
            if (leftRight < 0) {
                smaller = left;
            } else {
            ...
            //Swap it if root is larger than the smaller.
            int topSmaller = order.compare(array.entry(top),
                    array.entry(smaller));
            if (topSmaller > 0) {
            ...
            int topLeft = order.compare(array.entry(top), array.entry(left));
            if (topLeft > 0) {
                //compare the top and left, if top is bigger than left, swap.
    ...
    public final T removeFirst() {
        ...
        //Remove the first entry of the array, which is the root.
        T removed = this.heap.entry(0);
        //Replace the root with the last available entry in the array
        //which is the rightMost child of the bottom line.
        T rightMost = this.heap.entry(this.heapSize - 1);
        this.heap.setEntry(0, rightMost);
        //decrement the heapSize
        this.heapSize--;
        //SiftDown method to make it a heap again.
